...
>(({ value }, ref) => {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
                  ...
                  pendingDisplayEntries.map((entry, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3 text-right">
                        <button
                          className="rounded border border-border-dark px-2 py-1 text-[10px] uppercase text-slate-400 hover:text-rose-300"
                          type="button"
                          onClick={() =>
                            setPendingEntries((previous) =>
      ...
      <div className="border-t border-border-dark bg-surface-dark/40 px-6 py-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          <label className="flex flex-col gap-2 text-xs text-slate-300">
            Fields (JSON)
            <textarea
              className="min-h-[120px] w-full rounded-lg border border-border-dark bg-background/40 p-3 font-mono text-xs text-slate-200 focus:ring-2 focus:ring-primary/50"
              value={entryFields}
              onChange={(event) => setEntryFields(event.target.value)}
            />
          </label>
          {parseError && (
