...
public static class GetServerInfoEndpoint
{
    public static void Map(RouteGroupBuilder group)
    {
        group.MapGet("{connectionName}/info", HandleAsync)
            .Produces<ApiResponse<RedisServerInfo>>(StatusCodes.Status200OK)
        ...
        CancellationToken cancellationToken)
    {
        return await AZRedisEndpointHelpers.WithConnection(
            ...
            connectionName,
            cancellationToken,
            connection => serverService.GetInfoAsync(connection, cancellationToken));
    }
}
